<div class="flex flex-col h-screen bg-gray-100 p-4">
    <div class="max-w-3xl mx-auto w-full bg-white rounded-2xl shadow-lg flex flex-col h-full overflow-hidden">
            ...
            @forelse ($messages as $message)
                <div class="flex {{ $message->sender === 'admin' ? 'justify-end' : 'justify-start' }}">
                    <div class="px-4 py-2 rounded-2xl max-w-xs break-words
                        {{ $message->sender === 'admin' ? 'bg-indigo-500 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none' }}">
        ...
        {{-- Input --}}
        <form wire:submit.prevent="sendMessage" class="bg-white border-t border-gray-200 p-4 flex items-center space-x-3">
            <input type="text"
                   wire:model.defer="newMessage"
                   placeholder="Escribe un mensaje..."
                   class="flex-1 px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400">
            <button type="submit"
